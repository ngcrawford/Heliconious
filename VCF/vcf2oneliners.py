...
def process_vcf_slice(vcf_slice, position_data, start, stop, args):
    ...
    # SETUP MULTIPLE ALIGNMENT ARRAY
    numb_of_seqs = len(position_data._fields[9:])
    alignment = np.zeros((args.window_size,numb_of_seqs), np.string0)

    current_data = []
    if vcf_slice == ['']:
        return 'error'

    for line in vcf_slice:
        current_base = position_data._make(line.strip().split("\t"))
        base_calls = callSNPs(current_base, numb_of_seqs)
        current_data.append(base_calls.copy())
...
def main():
    ...
    # /Users/MullenLab/Desktop/Grad_Students/Nick/butterfly_practice/multiallelic_matt
    # 32-butterflies.multiallelic.all-chrms.NCBI-ids.gatk.vcf.gz
    ...
    vcf = args.input[0]
    fout = args.output
    slices = header_slices(vcf)

    vcf_file = gzip.open(vcf,'r')
    position_data, chrm_data = makeDataTuple(vcf_file)
    vcf_file.close()

    for c_count, chrm in enumerate(slices.keys()):
        for s_count, s in enumerate(slices[chrm]):
            chrm = 'HE670298'
            start = s
            stop = s + window_size
            vcf_slice = slice_vcf(vcf, chrm, start, stop)
            vcf_slice = vcf_slice.strip().split("\n")
            oneliner = process_vcf_slice(vcf_slice, position_data, start, stop, args)
            if oneliner != 'error':
                sys.stdout.write(oneliner+"\n")
        #     if s_count == 3: break
        # if c_count == 1: break

    #informative_sites = parse_window_vcf(vcf, args.start, args.stop, args.window_size, args.chromosome, fout)
    # vcf.close()
    # return informative_sites
